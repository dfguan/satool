...
{
	uint32_t vid = get_name2id(g, vname);
	if (~vid) {
		asm_t *as = &g->as.asms[g->as.casm];
		vertex_t *vt = g->vtx.vertices;
		uint32_t _pid = vt[vid>>1].pv;
		path_t *pt = &g->pt.paths[_pid];
		uint32_t i;
		for ( i = 0; i < pt->n; ++i ) if ((pt->ns[i] >> 1) == vid) {
			*pid = _pid;
			*vidx = i;
			return 0;
		}
	}
	fprintf(stderr, "[W::%s] %s not found!\n",__func__, vname); return 1;
}
